...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.isAuthenticated()) {
    console.log('### passport-remember-me user isAuthenticated')
    return this.pass();
  } else {
    console.log('### passport-remember-me use is not authenticated')
  }

  var token = req.cookies[this._key];

  console.log('### passport-remember-me token from cookie', token)
  ...
  if (!token) {
    console.log('### passport-remember-me invalid token from cookie')
    return this.pass();
  }
  ...
  function verified(err, user, info) {
    console.log('### passport-remember-me token call verified')
    if (err) {
      console.log('### passport-remember-me token verified error')
      return self.error(err);
    } else {
      console.log('### passport-remember-me token verified no error')
    }
    ...
    if (!user) {
      ...
      console.log('### passport-remember-me clear cookie')
      console.log('### passport-remember-me clearCookie -> return self.pass')
      return self.pass();
    }
    ...
    function issued(err, val) {
      console.log('### passport-remember-me token call issued')
      if (err) {
        console.log('### passport-remember-me token issued error', err)
        return self.error(err);
      }

      res.cookie(self._key, val, self._opts);
      console.log('### passport-remember-me set cookie', self._key, val, self._opts)

      console.log('### passport-remember-me return self.pass', user, info)
      return self.success(user, info);
    }

    console.log('### passport-remember-me token call issued -> self._issue', user, issued)
    self._issue(user, issued);
  }

  console.log('### passport-remember-me token call -> self._verify', token, verified)
